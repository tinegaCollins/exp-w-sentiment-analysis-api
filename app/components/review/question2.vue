<template>
    <div>
        <div class="flex items-center">
            <h1 class="text-xl font-bold">Question 2</h1>
            <p class="border border-black rounded-full grid place-items-center h-6 ml-auto w-6">?</p>
        </div>
        <h1 class="mt-3">Explain your overall experience of the product.</h1>
        <h2>Optional</h2>
        <div>
            <label for="message" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your
                message</label>
            <textarea id="message" rows="4" v-model="t"
...
let t = ref("");
const next = () => {
    review.setOverallExperience(t.value);
    emit("next");
}
const previous = () => {
    emit("previous");
